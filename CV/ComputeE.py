...
T =args.T
os.chdir('/scratch/diharcej/LJ/{}_trj/last_trj/'.format(T))
#os.mkdir('./q6_colvar')


def doE(input_file, cutoff_q6 = 0.25, format="lammps-dump"):
    output_file=input_file.replace(".lammpstrj.snap.","_").replace(".dat","_BIS.lammpstrj")
    sys = pc.System()
    sys.read_inputfile(input_file, format=format)
    sys.calculate_energy(species=None, pair_style='lj/cut',
                     pair_coeff='* * Mo.set Mo', mass=1)
    atoms = sys.atoms
