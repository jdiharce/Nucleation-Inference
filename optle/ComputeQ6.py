...
T =args.T
T = str(T).split('.')[0] + str(T).split('.')[1]
os.chdir('/home/sorbonne/optLE/mesu_results/AS_{}/accepted_trj_300/start_trj/'.format(T))
#os.mkdir('./q6_colvar')
    ...
    if 'snap' not in input_file and 'BIS' not in input_file:
        ...
        with open(filename, 'w') as file:
            ...
            for item in q6clusters:
                ...
                count += 1
files1 = glob.glob("*snap")
files2 = glob.glob("*BIS")
for file in files1:
    os.remove(file)
for file in files2:
    os.remove(file)
